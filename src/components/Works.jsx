...
const Works = () => {
    ...
    return (<>
        ...
        <div
            className={`mt-20 max-h-max overflow-hidden flex flex-wrap gap-7 transition-all duration-[0.9s] ${showMore ? 'h-[10000px]' : 'h-[65vh] '}`}>
            {projects.map((project, index) => (
                <ProjectCard key={`project-${index}`} {...project} index={index}/>))}
        </div>

        <button
            className="bg-tertiary flex items-center p-5 py-3 w-fit m-auto rounded-[90px]"
            onClick={() => {
